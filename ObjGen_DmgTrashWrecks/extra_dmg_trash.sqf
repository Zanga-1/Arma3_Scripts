...
// changes made based on Tanoa, might have to add object types in "forEach nearestTerrainObjects" on other maps
// objects have 50% of being destroyed
...
if (isServer) then {
    ...
    {
        _pos = getPos _x;
        if (!(isObjectHidden _x) && {damage _x < 1}) then {
            _damage = (99 + floor (random 2)) / 100;
			_x setDamage [_damage, false]
        };
		
		sleep 0.0001;
    } forEach nearestTerrainObjects [_centerPos, ["BUILDING","CROSS","FORTRESS","LIGHTHOUSE","QUAY","TRANSMITTER","BUNKER","FENCE","FOUNTAIN","HOSPITAL","POWERWAVE","WATERTOWER","CHURCH","HOUSE","POWERWIND","RUIN","HIDE",
												  "FOREST BORDER","FUELSTATION","POWER LINES","STACK","VIEW-TOWER","BUSSTOP","FOREST SQUARE","POWERSOLAR","TOURISM","WALL","CHAPEL"], _width, false];
